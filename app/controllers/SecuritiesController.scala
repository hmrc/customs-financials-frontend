...
import actionbuilders.{AuthenticatedRequestWithSessionId, IdentifierAction, SessionIdAction}
import config.{AppConfig, ErrorHandler}
import domain.FileRole.SecurityStatement
import domain.{EoriHistory, SecurityStatementFile, SecurityStatementsByPeriod, SecurityStatementsForEori}
import play.api.i18n.I18nSupport
import play.api.mvc.{Action, AnyContent, MessagesControllerComponents}
import play.api.{Logger, LoggerLike}
import services.{ApiService, DocumentService}
...
class SecuritiesController @Inject()(val authenticate: IdentifierAction,
  ...
  private def getStatements(historicEori: EoriHistory)(implicit req: AuthenticatedRequestWithSessionId[_]): Future[SecurityStatementsForEori] = {
    documentService.getSecurityStatements(historicEori.eori)
      ...
      .map {
        case (current, requested) => domain.SecurityStatementsForEori(historicEori, current, requested)
      }
  }
