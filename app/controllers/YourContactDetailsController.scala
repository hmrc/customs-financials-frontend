...
class YourContactDetailsController @Inject()(authenticate: IdentifierAction,
  ...
  def onPageLoad(): Action[AnyContent] = authenticate async { implicit request =>
    ...
    val localSessionId = hc.sessionId.getOrElse({log.error("Missing SessionID"); SessionId("Missing Session ID")})

    localSessionId match {
      case a => sessionCacheConnector.getSessionId(localSessionId.value) match {
        case sessionCache if sessionCache.value == localSessionId.value => for {
