...
import uk.gov.hmrc.customs.financials.domain.{AuditEori, AuditModel, SignedInUser}
...
class AuthActionHelper @Inject()(dataStoreService: DataStoreService,
  ...
  def authenticatedRequest[A](credentials: Option[Credentials],
    ...
    for {
      allEoriHistory <- dataStoreService.getAllEoriHistory(eori)
