...
class DutyDefermentAccountsViewModelSpec extends SpecBase {
  ...
  "View model" should {
    ...
    "contain correct title and inaccurate balances msg" when {
      ...
      "there are more than one DD account" in new Setup {
        ddAccountsViewModelWithMultipleAccounts.titleMsg mustBe msgs("cf.customs-financials-home.duty-deferment.title2")
        shouldContainInaccurateBalancesMsg(ddAccountsViewModelWithMultipleAccounts)
      }
    }
    ...
    "contain correct account section rows" when {
      ...
      "there are more than one DD accounts" ignore new Setup {
        ddAccountsViewModelWithMultipleAccounts.accountSectionRows mustBe
          Seq(expectedAccountSectionRow(finHomeModel1), expectedAccountSectionRow(finHomeModel1))
      }
    }
  }
  ...
  trait Setup {
    ...
    val amountLimit = BigDecimal(100.00)
    implicit val app = application().build()
    ...
    val ddAccountsViewModelWithMultipleAccounts: DutyDefermentAccountsViewModel =
      DutyDefermentAccountsViewModel(finHomeModel1)
