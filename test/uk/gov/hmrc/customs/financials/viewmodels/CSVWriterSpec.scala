...
class CSVWriterSpec extends SpecBase    {
  ...
  "CSVWriter" should {
    ...
    "prepend header row with case class field names if requested" in {
      ...
      records.toCSVWithHeaders() must be (""""columnA","columnB","columnC"
          |"A","B","C"
          |"X","Y","Z"""".stripMargin)
    }
    ...
    "map header names using supplied function" in {
      ...
      records.toCSVWithHeaders(mappingFn) must be (""""COLUMNA","COLUMNB","COLUMNC"
          |"a","b","c"""".stripMargin)
    }
    ...
    "append footer row with text" in {
      ...
      records.toCSVWithHeaders(footer = Some("footer text")) must be (""""columnA","columnB","columnC"
                                            |"A","B","C"
                                            |"X","Y","Z"
                                            |
                                            |
