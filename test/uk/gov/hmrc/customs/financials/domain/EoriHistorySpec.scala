...
class EoriHistorySpec extends SpecBase{
  ...
  "it" should {
    "parse ISO_LOCAL_DATE" in {
      ...
      eoriHistory.validFrom.get.toString must be ("2019-03-01")
    }
    ...
    "parse ISO_INSTANT" in {
      ...
      eoriHistory.validFrom.get.toString must be ("1985-03-20")
    }

    "obfuscate too short Eori" in {
      obfuscateEori("123") must be ("123")
    }

    "obfuscate tiny Eori History" in {
      obfuscateEori("1234") must be ("1234")
    }

    "obfuscate Eori History" in {
      obfuscateEori("12345") must be ("*2345")
    }
  }
}
